    ...
    try {
        ...
        if (user.isVerified === 'no') {
            ...
            sendMail({
                ...
                console.warn('📧 OTP email failed (non-critical):', err.message);
            });
...
// UPDATE RESUME
export const updateResumeService = async (userId, resume) => {
    try {
        if (!resume) {
            return {
                success: false,
        ...
        // Update all applications for this user with the new resume
